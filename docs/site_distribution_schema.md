...
modeled FBS team-game. It represents the discrete rank support as unit-width
rank cells and divides the full rank axis into 40 equal-width intervals;
probability mass is split between display intervals according to cell overlap,
rather than assigning each rank wholly to one bucket. The future-game display
uses 40 CDF-derived integer
...
All display weights are presentation-only; exact completed-game summaries,
percentiles, grades, and ranking outputs remain authoritative outside these
compressed arrays.
