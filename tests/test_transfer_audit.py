...
def test_audit_is_explicit_about_aliases_ambiguity_and_normalization() -> None:
    ...
    assert by_player["G NoStats"]["d5_resolution_category"] == D5_CATEGORY_ZERO
    assert by_player["H Linemen"]["d5_resolution_category"] == D5_CATEGORY_UNDETERMINED
    assert (
        by_player["I Incidental"]["d5_resolution_category"] == D5_CATEGORY_UNDETERMINED
    )
    assert by_player["J NonFbs"]["d5_resolution_category"] == D5_CATEGORY_UNDETERMINED
    assert by_player["D Player"]["in_model_relevant_population"] is False

    season = audit["season_rows"][0]
    assert season["incoming_fbs_transfers"] == 9
    ...
    assert season["d5_legitimate_zero_or_non_applicable"] == 2
    assert season["d5_resolution_failures"] == 2
    assert season["d5_applicability_unknown"] == 4
    assert season["d5_resolved_rate_among_determined"] == pytest.approx(0.6)
