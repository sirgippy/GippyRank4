...
def test_candidate_set_keeps_c10_fixed_and_does_not_search_rolling_results() -> None:
...
def test_training_coverage_excludes_target_and_counts_raw_observations() -> None:
    ...
    assert result["n_team_seasons_with_transfer_net_prior_usage"] == 1
    assert result["n_team_seasons_with_all_observed_transfer_production"] == 1
    assert result["fraction_with_all_observed_transfer_production"] == 0.5
